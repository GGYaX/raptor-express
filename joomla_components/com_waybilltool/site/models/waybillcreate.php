...
class WaybillToolModelWaybillCreate extends JModelItem
{
        ...
        try {
            ...
            if (isset($priceCalculed['error'])) {
                ...
                if ($data['product'] == 'ENA' || $data['product'] == 'ENO') {
                    // ems
                    if (floatval($walletAmount['emsAmount']) <
                             floatval($priceCalculed['result'])) {
                        $canInsert = false;
                    }
                } else {
                    // la poste
                    if (floatval($walletAmount['laposteAmount']) <
                             floatval($priceCalculed['result'])) {
                        $canInsert = false;
                    }
                }
            ...
            if ($canInsert == true) {
                $resAddrSent = $this->insertNewAddress($data["uid"],
                        $data["name_send"], $data["addr_send_stre"],
                        $data["addr_send_post"], $data["addr_send_city"],
                        $data["addr_send_stat"], $data["addr_send_cnty"],
                        $data["phone_send"]);
                $resAddrRecv = $this->insertNewAddress($data["uid"],
                        $data["name_recv"], $data["addr_recv_stre"],
                        $data["addr_recv_post"], $data["addr_recv_city"],
                        $data["addr_recv_stat"], $data["addr_recv_cnty"],
                        $data["phone_recv"]);
                ...
                $helper = new WaybillParamsCheckerHelper();
                $orderMappingId = $helper->mappingId($resOrder['id']);
                $filenameRecto = $orderMappingId . '_recto';
                $filenameVerso = $orderMappingId . '_verso';
                $resIDCards = $this->insertNewIDCards($data["uid"],
                        $filenameRecto, $filenameVerso, $resOrder["id"]);
                if ($GLOBALS['WAYBILLTOOL_DEBUG']) {
                    echo '<pre>';
            ...
            return array(
                    'ok' => false,
                    'msg' => JText::_('COM_WAYBILLTOOL_VIEW_FONT_OUT_OF_AMOUNT')
            );
        } catch (Exception $e) {
            /* DEBUG */
            if ($GLOBALS['WAYBILLTOOL_DEBUG'])
    ...
    private function insertNewAddress ($address_user_id, $address_firstname,
            $address_street, $address_post_code, $address_city, $address_state,
            $address_country, $address_telephone)
    {
        if ($GLOBALS['WAYBILLTOOL_DEBUG'])
            echo "写入Address<br>";
        ...
        $values = array(
                $db->quote($address_user_id),
                $db->quote($address_firstname),
                $db->quote($address_street),
                $db->quote($address_post_code),
                $db->quote($address_city),
                $db->quote($address_telephone),
                $db->quote($address_state),
                $db->quote($address_country)
        );
            ...
            $comment, $package_stat = "DDJ")
    {
        ...
        $values = array(
                $db->quote($sender_id),
                $db->quote($recipient_id),
                $db->quote($package_stat),
                $db->quote($insured_amount),
                $db->quote($express_mode),
                $db->quote($weight),
                $db->quote($height),
                $db->quote($length),
                $db->quote($wide),
            ...
            $express_type, $payment_stat = "YFK", $media_code = "ONL")
    {
        ...
        $values = array(
                $db->quote($payment_stat),
                "NOW()",
                $db->quote($payment_amount),
                $db->quote($media_code),
                $db->quote($package_id),
                $db->quote($client_id),
                $db->quote($express_type)
        );
            ...
            $filename_verso, $order_id)
    {
        ...
        $values = array(
                $db->quote($user_id),
                $db->quote($filename_recto),
                $db->quote($filename_verso),
                $db->quote($order_id)
        );
     ...
     * 计算价格
     */
    private function calculPrice ($express_type, $high = 0, $length = 0,
        ...
        if ($high == 0 || $length == 0 || $width == 0 || $weight == 0) {
            ...
            if ($express_type = 'EMS') {
                ...
                if ($weightCalculed > 20) {
                    $toReturn = array(
                            'error' => JText::_(
                                    'COM_WAYBILLTOOL_VIEW_FONT_OUT_OF_BOUND')
                    );
                } else {
                    $toReturn = array(
                            'result' => $this->emsArray[$weightCalculed . 'KG']
                    );
                }
            } else
                if ($express_type = 'ENA') {
                    ...
                    if ($express_type = 'LAP') {
                        if ($weightCalculed > 15) {
                            ...
                            $toReturn = array(
                                    'result' => $this->laposteArray[intval(
                                            $weightCalculed) . 'KG']
                            );
                        }
                    } else {
