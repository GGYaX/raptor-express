		...
		foreach($data as $c)
		{
			...
			elseif ($this->vm_version==2)
			{
				if( !empty($c->file_meta)) {
					$doSql2 = true;
					$sql2 .= $sep2."('','','".$c->file_meta."','category',".$id.')';
					$sep2 = ',';
					$file_name = str_replace('\\','/',$c->file_meta);
					if( strpos($file_name,'/') !== false ) {
						$file_name = substr($file_name, strrpos($file_name,'/'));
					}
					$this->copyFile($this->copyCatImgDir,$c->file_meta, $this->options->uploadfolder.$file_name);
	...
	function importProducts() {
		...
		elseif ($this->vm_version==2) //OK
		{
			$this->db->setQuery(
				...
				"WHERE vmp.virtuemart_product_id > ".$offset." AND hkprod.hk_id IS NULL AND (vmm.file_meta IS NOT NULL) AND vmm.file_meta <> '' ".
			...
			$sql4 = 'INSERT IGNORE INTO `#__hikashop_file` (`'.implode('`,`',array_keys($data)).'`) '.
				...
				"INNER JOIN `".$this->vmprefix."virtuemart_medias` vmm ON vmpm.virtuemart_media_id = vmm.virtuemart_media_id ".
				'WHERE vmp.virtuemart_product_id > '.$this->options->last_vm_prod.' AND (vmm.file_meta <>'." '') ";
