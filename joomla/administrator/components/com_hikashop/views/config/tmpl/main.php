			...
			<legend><?php echo JText::_( 'HIKASHOP_ORDER' ); ?></legend>
				<table class="admintable table" style="width:100%" cellspacing="1">
					...
					<tr>
						...
						<td>
							<input type="text" class="inputbox" id="payment_capture_order_status" name="config[payment_capture_order_status]" value="<?php echo @$this->config->get('payment_capture_order_status', 'shipped'); ?>" />
							<?php
								echo $this->popup->display(
									'<img src="'. HIKASHOP_IMAGES.'edit.png" alt="'.JText::_('HIKA_EDIT').'"/>',
									'CANCELLABLE_ORDER_STATUS',
									'\''.hikashop_completeLink("category&task=selectstatus&control=payment_capture_order_status",true).'&values=\'+document.getElementById(\'payment_capture_order_status\').value',
									'link_payment_capture_order_status',
									760, 480, '', '', 'link',true
								);
								?>
						</td>
				...
				<fieldset class="adminform">
					...
					<table class="admintable table" style="width:100%" cellspacing="1">
						...
						<tr>
							...
							<td>
								HikaShop <?php $this->config->get('level').' '.$this->config->get('version'); ?> [1411161808]
							</td>
						</tr>
					</table>
				</fieldset>
