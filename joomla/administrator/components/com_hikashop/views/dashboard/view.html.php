...
class dashboardViewDashboard extends hikashopView {
	...
	function links(){
		...
		$desc['config'] = '<ul><li>'.JText::_('CONFIG_DESC_HIKASHOP_CONFIG').'</li><li>'.JText::_('CONFIG_DESC_HIKASHOP_MODIFY').'</li><li>'.JText::_('CONFIG_DESC_EMAIL');
		if(!hikashop_level(1)){
			$desc['config'] .= ' <small style="color:red">'.JText::_('ONLY_COMMERCIAL').'</small>';
		}
		$config =& hikashop_config();
		if(hikashop_isAllowed($config->get('acl_config_view','all'))) $desc['config'] .= '</li><li>'.JText::_('CONFIG_DESC_HIKASHOP_PLUGIN').'</li></ul>';
