	...
	<?php
		if(empty($this->element->variants) || $this->params->get('characteristic_display')=='list'){
			if(hikashop_level(1) && !empty($this->element->options)){
				...
				echo '
				<input type="hidden" name="hikashop_price_product" value="'.$this->element->product_id.'" />
				<input type="hidden" id="hikashop_price_product_'.$this->element->product_id.'" value="'.$priceUsed.'" />
				<input type="hidden" id="hikashop_price_product_with_options_'.$this->element->product_id.'" value="'.$priceUsed.'" />';
			}
		} else {
